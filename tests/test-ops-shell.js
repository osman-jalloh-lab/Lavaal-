...
async function run() {
  ...
  check('nav lists all ten areas', NAV.length === 10
    && NAV.map((item) => item.id).join(',') === 'dashboard,profile,chat,memory,inbox,calendar,kits,map,tasks,routines');
...

  {
    ...
    check('dashboard nav links all ten areas', NAV.every((item) => html.includes(`href="${item.href}"`)));
