...
| 9 | Public catalog / Request Quote unchanged; `/ops` not in public nav | **Passed** | `index.html` still quote-first (“Enterprise IT Hardware”, nav CTA `Request Quote` → `#signup`). No `href="/ops"` in public HTML (`privacy.html` / `terms.html` likewise). `vercel.json` still rewrites `/` to `index.html`. Quote API (`api/quote.js`) unchanged in this PR track. |
