...
function renderProduct(product) {
  ...
  const thumbnails = images.map((image, index) => `<button class="thumb${image === primary ? ' active' : ''}" data-image="${reviewAssetPrefix}${esc(image)}" data-alt="${esc(`${productAlt} — image ${index + 1}`)}" aria-label="View image ${index + 1} of ${images.length}"><img src="${reviewAssetPrefix}${esc(image)}" alt=""></button>`).join('');
  const specs = keySpecs(product).map(spec => `<li>${esc(spec)}</li>`).join('');
  return `<article class="product"><img class="primary" src="${reviewAssetPrefix}${esc(primary)}" alt="${esc(`${productAlt} — image 1`)}"><div class="thumbs">${thumbnails}</div><h2>${esc(product.brand)} — ${esc(product.name)}</h2><p>Icecat ID: ${esc(product.sourceProductId)} · MPN: ${esc(product.mpn ?? 'not provided')} · GTIN: ${esc(product.gtin ?? 'not provided')}</p><ul>${specs}</ul><label><input type="checkbox"> PASS</label> <label><input type="checkbox"> REVIEW</label></article>`;
}
