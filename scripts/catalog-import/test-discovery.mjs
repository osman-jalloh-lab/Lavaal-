...
assert.equal(records.length, 9); assert.deepEqual(records[0].gtins, ['1000000000001']); assert.equal(records[0].mpn, 'FAKE-TAB-A');
...
const discovery = discoverRecords(records.filter(record => Number(record.icecatId) <= 9007), options);
...
const topKRecords = records.filter(record => ['9008', '9009'].includes(record.icecatId));
const topKOptions = { ...options, onMarketOnly: true, maxCandidatesPerTarget: 1 };
const topKFirst = discoverRecords(topKRecords, topKOptions); const topKSecond = discoverRecords(topKRecords, topKOptions);
assert.equal(topKFirst.targets[0].candidates.length, 1); assert.equal(topKFirst.targets[0].candidates[0].icecatId, '9009');
assert.equal(topKFirst.targets[0].candidates[0].selected, false); assert.deepEqual(topKFirst, topKSecond);
